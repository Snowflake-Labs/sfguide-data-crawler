...
def get_databases():
    ...
    return pd.DataFrame([{'DATABASE_NAME': '<Select>'}]) ._append(session.sql("""
...
def get_table_catalog(databasename):
    df = session.sql (f"SELECT distinct COMMENT, table_catalog, table_schema, table_name, table_owner, row_count FROM {databasename}.information_schema.tables where table_schema !='INFORMATION_SCHEMA'")
    return df.toPandas()
...
def get_column_data(tablename):
    df = session.sql(f"select TABLE_NAME, COLUMN_NAME, COMMENT from information_schema.columns where table_name= '{tablename}'")
    return df.toPandas()

# ユーザーアクセス情報を取得する関数（キャッシュ付き）
@st.cache_data()
def get_useraccess(tablename):
    # PARAM_TABLE_ACCESS が定義されているか確認
    if 'PARAM_TABLE_ACCESS' not in globals():
        st.error("PARAM_TABLE_ACCESS が定義されていません。")
        return pd.DataFrame()
    df = session.sql(f"select user as USER_NAME, name as tablename from {PARAM_TABLE_ACCESS} join (select distinct role, grantee_name as user from SNOWFLAKE.ACCOUNT_USAGE.GRANTS_TO_USERS) user_table on {PARAM_TABLE_ACCESS}.GRANTEE_NAME = user_table.role where tablename='{tablename}';")
    return df.toPandas()

# 機密カラム情報を取得する関数（キャッシュ付き）
@st.cache_data()
def get_sensitive_column(databasename, tablename):
    df = session.sql(f"select COLUMN_NAME, TAG_NAME, TAG_VALUE from snowflake.account_usage.tag_references where OBJECT_DATABASE = '{databasename}' and OBJECT_NAME ='{tablename}'")
    return df.toPandas()
...
def get_table_context(table_name, column_data):
    context = f"""
        ...
        また、対象のテーブルが持つ列情報は<columns>"{str(column_data)}"</columns>です。
    """
    return context
...
if filter_database != '<Select>':
    database_name = filter_database.replace('(','').replace(')','')
    ...
    with st.spinner('Retrieving the list of objects...'):
        ...
        for index, row in table_catalog.iterrows():
            ...
            if get_data_details:
                ...
                table_name = row['TABLE_NAME'] # rowから直接取得
                ...
                with st.expander("LLMを使ったテーブルの詳細分析"):
                    ...
                    # st.write(prompt)
                    response = get_response(session, prompt) # st.session_state.messages ではなく prompt を渡す
                    if response:
